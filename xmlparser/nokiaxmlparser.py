    ...
    with gzip.open(BASE_DIR+separator+src, 'rb') as src_file:
        for line in src_file:
            ...
            if line.find('</'+mobj+'>') >= 0:
                ...
                is_mo = False
                if (file_cnt % size) == 0:
                    print(src+': Making ' + str(int(file_cnt / size)) + '.xml')
                    with open(BASE_DIR + separator + path + separator + str(int(file_cnt / size)) + '.xml', 'w+', encoding="utf-8") as fp:
...
class ManagedObjectHandler(ContentHandler):
    ...
    def startElement(self, name, Attributes):
        ...
        if name == 'managedObject':
            ...
            self.parameters.append('version_attr')
        ...
        if val:
            try:
                if len(val) == 1:
                    ...
                    q = 'CREATE TABLE ' + key + str(tuple(val)) + ';'
                    cursor.execute(q)
            except:
...
if __name__ == '__main__':
    ...
    raw_xml_databases = [SOURCE_FILE]
    if IS_DIRECTORY_PARSE:
        raw_xml_databases = []
        for raw_db in os.listdir(BASE_DIR+separator+RAW_XML_DB_DIR):
            if raw_db.find('.xml.gz')>=0:
                raw_xml_databases.append(RAW_XML_DB_DIR+separator+raw_db)
    ...
    for source_file in raw_xml_databases:
        ...
        for xml_file in os.listdir(xml_file_path):
            parse_count += 1
            print('['+source_file+':'+'] '+str(parse_count)+' - Working With '+xml_file)
        ...
        clean_db(BASE_DIR + separator + DB_NAME + '_' + source_file.split('\\')[-1].split('.')[0]+'.sqlite')
        create_tables(BASE_DIR + separator + DB_NAME + '_' + source_file.split('\\')[-1].split('.')[0]+'.sqlite')
        insert_data(BASE_DIR + separator + DB_NAME + '_' + source_file.split('\\')[-1].split('.')[0]+'.sqlite')
    ...
    if not IS_DIRECTORY_PARSE:
        if IS_ZIP_DB:
            with open(BASE_DIR+separator+DB_NAME + '_' + source_file.split('\\')[-1].split('.')[0]+'.sqlite', 'rb') as f_in, gzip.open(BASE_DIR+separator+time.strftime("%y%m%d")+'.sqlite.gz', 'wb') as f_out:
                f_out.writelines(f_in)
            print("DB ZIP DONE!")
            
            if IS_WEEKLY_BAKUP:
                myhelper.clean_dir(BASE_DIR+separator+'backup'+separator+time.strftime("%A"))
                shutil.copy2(BASE_DIR+separator+time.strftime("%y%m%d")+'.gz', 
                            BASE_DIR+separator+'backup'+separator+time.strftime("%A")+separator+time.strftime("%y%m%d")+'.gz')
